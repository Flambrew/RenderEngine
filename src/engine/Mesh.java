...
public class Mesh {
	...
	public void orderByDistanceFrom(Vector3 point) {
		triangles.sort(new Comparator<Triangle>() {
			public int compare(Triangle a, Triangle b) {
				return Vector3.distanceBetween(point, Vector3.centroid(a.asArray())) < Vector3.distanceBetween(point, Vector3.centroid(b.asArray())) ? 1 : -1;
			}
		});
	}
	...
	public Mesh translate(Matrix4... translation) {
		for (Matrix4 matrix : translation) {
			for (Triangle triangle : triangles) {
				triangle.a = triangle.a.transform(matrix);
				triangle.b = triangle.b.transform(matrix);
				triangle.c = triangle.c.transform(matrix);
			}
		}
		return this;
	}
